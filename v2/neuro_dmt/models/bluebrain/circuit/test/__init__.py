...
class CircuitAnalysisTest(WithFields):
    ...
    def test_get_measurement(self, circuit_model):
        """
        Measurement returned by an analysis should be a pandas data-frame.
        """
        measurement = self.analysis.get_measurement(circuit_model)
        assert isinstance(measurement, pd.DataFrame)
        return measurement

    def test_get_report(self, circuit_model):
        """
        Report generated by analysis should have figures.
        """
        report = self.analysis.get_report(circuit_model)
        assert isinstance(report, Report)
        assert len(report.figures) > 0
        assert hasattr(report, "measurement")
        return report

    def test_call_analysis(self, circuit_model):
        """
        The analysis should be a callable, and return a report when called.
        """
        report = self.analysis(circuit_model)
        assert isinstance(report, Report)

    def test_post_report(self, circuit_model):
        """
