...
class Document(Analysis):
    """
    Document your analysis while you code it!
    """
    pass
...
class Chapter(Document):
    ...
    @classmethod
    def is_section(cls, attribute):
        """
        """
        try:
            value_attribute = getattr(cls, attribute)
        except AttributeError:
            return False

        try:
            return issubclass(value_attribute, Section)
        except TypeError:
            return False
    ...
    def __call__(self, adapter, model, *args, **kwargs):
        """
        Produce this chapter for (adapter, model)
        """
