...
document = DocumentBuilder("Circuit Composition",
                           author=Author.zero)
@document.abstract
def _():
    """
...
analyses = Record(
    cell_density=document.cell_density,
    inhibitory_fraction=document.inhibitory_fraction)


from dmt.analysis.literate import Document, Section


class CircuitComposition(Document):
    """
    Cortical areas such as the \\model{brain_region} are composed of layers of
    cells with different cell densities.
    Here we analyze circuit composition of cortical layers, focusing on total
    density and the fraction of inhibitory neurons in each layer.
    """
    class Introduction(Section):
    class CellDensity(Section):
